"""
Temporal activity wrapper and workflow proxy for assembly specification repository.

This module contains:
1. The @temporal_activity_registration decorated class that wraps the pure backend
   MinioAssemblySpecificationRepository as Temporal activities (for worker use only)
2. The @temporal_workflow_proxy decorated class that automatically generates
   workflow methods that delegate to activities

The activity names follow the pattern: julee.assembly_specification_repo.minio.{method}
"""

from util.repos.temporal.decorators import temporal_activity_registration, temporal_workflow_proxy
from julee_example.repositories.minio.assembly_specification import MinioAssemblySpecificationRepository
from julee_example.repositories.assembly_specification import AssemblySpecificationRepository
...
class TemporalMinioAssemblySpecificationRepository(MinioAssemblySpecificationRepository):
...
class WorkflowAssemblySpecificationRepositoryProxy(AssemblySpecificationRepository):
    """
    Workflow implementation of AssemblySpecificationRepository that calls activities.
