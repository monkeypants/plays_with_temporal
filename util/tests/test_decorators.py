...
def test_new_class_parameter() -> None:
    """Test new_class parameter preserves original class when True."""

    # Create a fresh mock class for testing
    class TestRepository:
        async def test_method(self) -> str:
            return "test"

    # Test default behavior (new_class=False) - modifies original
    original_has_activity_before = "__temporal_activity_definition" in dir(
        TestRepository.test_method
    )

    ModifiedRepo = temporal_repository("test.modify_original")(TestRepository)

    original_modified = "__temporal_activity_definition" in dir(
        TestRepository.test_method
    )

    assert ModifiedRepo is TestRepository  # Same class object
    assert not original_has_activity_before
    assert original_modified  # Original was modified

    # Create another fresh class for new_class=True test
    class FreshRepository:
        async def another_method(self) -> str:
            return "fresh"

    # Test new_class=True behavior (preserves original)
    NewClassRepo = temporal_repository(
        "test.new_class", new_class=True
    )(FreshRepository)

    # Verify original is preserved
    original_preserved = "__temporal_activity_definition" not in dir(
        FreshRepository.another_method
    )
    new_class_has_activity = "__temporal_activity_definition" in dir(
        NewClassRepo.another_method
    )

    assert NewClassRepo is not FreshRepository  # Different class objects
    assert original_preserved  # Original unchanged
    assert new_class_has_activity  # New class has activities
    assert NewClassRepo.__name__ == "TemporalFreshRepository"
    assert issubclass(NewClassRepo, FreshRepository)  # Inherits from original
